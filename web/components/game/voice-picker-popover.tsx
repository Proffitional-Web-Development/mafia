...
export function VoicePickerPopover({
  ...
  const handlePreview = useCallback((key: string, file: string) => {
    if (playingKey === key) {
      stopAudio();
      return;
    }
  ...
  const handleSend = useCallback(async (key: string) => {
    setSendingKey(key);
    stopAudio();
    try {
      await sendMessage({
    ...
    <>
      ...
      <div className="absolute bottom-full mb-2 start-0 z-50 w-72 rounded-xl border border-white/15 bg-surface/95 p-3 shadow-xl backdrop-blur-lg animate-in slide-in-from-bottom-2 fade-in duration-200">
        <div className="mb-2 flex items-center justify-between px-1">
          <span className="text-xs font-semibold text-text-secondary">{t("picker.title")}</span>
          <button 
             onClick={onClose}
        ...
        <div className="grid grid-cols-1 gap-1 max-h-60 overflow-y-auto pr-1">
          {VOICE_CLIPS.map((clip) => {
             ...
             return (
               <div key={clip.key} className="flex items-center justify-between gap-2 rounded-lg bg-white/5 p-2 transition-colors hover:bg-white/10">
                 <div className="flex items-center gap-2 overflow-hidden">
                    <button
                      ...
                      className={cn(
                        ...
                        isPlaying ? "bg-primary text-white border-primary" : "bg-white/5 text-text-secondary hover:bg-white/10"
                      )}
